...
func podID(element interface{}) string {
	...
	values := testutil.GetMetricLabelValue(el, "namespace", "pod")
	return strings.Join(values, "::")
}

func containerID(element interface{}) string {
	el := element.(*testutil.Sample)
	values := testutil.GetMetricLabelValue(el, "namespace", "pod", "container")
	return strings.Join(values, "::")
}
...
func boundedSample(lower, upper interface{}) types.GomegaMatcher {
	return gstruct.PointTo(gstruct.MatchAllFields(gstruct.Fields{
		...
		"Timestamp": gomega.WithTransform(testutil.GetModelTimeFunc(0),
			gomega.And(
